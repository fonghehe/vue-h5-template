...
- 不題外匯入 `vant/lib/index.css`（由 Resolver 統一管理 CSS 注入順序）

```ts
// bootstrap.ts（vant）
import "@vh5/styles/global";
// ✅ 不匯入 vant/lib/index.css，元件 CSS 由 VantResolver 按需注入
...
- **Snackbar（函式）**：在使用 Snackbar 的檔案中手動匯入 CSS
...
- **函式元件**（Toast/Notify/Dialog/ImagePreview）：在 `bootstrap.ts` 手動匯入 CSS
