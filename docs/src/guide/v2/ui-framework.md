...
The themed floating AI button opens the lazy `/ai/chat` route above the bottom tabs. It is hidden on Chat, Login, Cart and Details to avoid competing with input or checkout controls. Streaming, Stop, Regenerate, Copy and sanitized Markdown use the same implementation in all three apps.
