...
1. 喺 `packages/api/src/<domain>.ts` 中添加 DTO 同接口定義。
2. 如需領域轉換，喺 `packages/services/src/<domain>.service.ts` 中添加。
3. 喺特性套件嘅 Composable 中封裝（`packages/features/<domain>/composables/`）。
4. 喺視圖中使用該 Composable。
