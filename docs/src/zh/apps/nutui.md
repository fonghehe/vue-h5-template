...
三套应用统一 Home、List、Member、Examples Tab，对应 `/home`、`/list`、`/member`、`/examples`。业务页来自 `packages/mobile-ui`，原生导航和 `/examples/components` 保留应用实现。默认英文，Home/Member 可切换中日文。主题色 AI 悬浮按钮打开 `/ai/chat`，在聊天/登录/购物车/详情隐藏。
