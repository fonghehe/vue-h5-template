...
[English](./README.md) | [简体中文](./README.zh-CN.md) | [日本語](./README.ja-JP.md) | 繁體中文 | [繁體中文(港)](./README.zh-HK.md)
...
- **獨立視覺系統**：Vant 藍、NutUI 紅、Varlet 紫各自使用完整設計 Token，避免跨框架色彩混用
- **型別安全基礎設施**：Strict TypeScript、明確 Lazy Route、OpenAPI 產生型別與統一 API Error
- **Streaming AI Chat**：供應商無關 SSE/ReadableStream、AbortController 與安全 Markdown
- **Server State**：TanStack Vue Query 提供 Cache、Mutation、Pagination、Infinite Query
- **真實 Mobile Pattern**：Error Boundary、Offline、Visual Viewport、下拉更新、Web Share、Clipboard 與持久化購物車
- **Vite 建置**：基於 Vite 的建置設定，支援自動匯入和元件自動註冊
- **UnoCSS**：原子化 CSS 引擎，全局使用 utility-first 風格撰寫樣式
- **狀態管理**：Pinia + 持久化插件，生產環境 AES 加密儲存
- **Mock 服务**：基於 Nitro 的 Mock 後端，提供登入認證、商品列表等介面
- **Eruda**：內置行動端除錯控制台，僅在非生產環境開啟
- **統一規範**：共享 ESLint / OxLint / Stylelint / Oxfmt / Commitlint 設定
- **行動適配**：postcss-mobile-forever 行動端適配方案（設計稿寬 375px，最大顯示寬 600px）
...
| Pinia               | 4.0   | 狀態管理           |
| Vue Router          | 5.0   | 路由管理           |
| TanStack Vue Query  | 5.x   | Server State 與 Cache |
| Axios               | 1.x   | 型別安全 REST Transport |
| Vue I18n            | 11.4  | 國際化             |
| Nitro               | 2.x   | Mock 伺服器        |
| NutUI               | 4.3   | UI 元件庫          |
| Vant                | 4.10   | UI 元件庫          |
...
開發環境預設使用內建 Nitro Mock。需要連接配套的 `vue-h5-template-business-service`（`:8002`）與 `vue-h5-template-ai-service`（`:8001`）時，先啟動兩個後端，再執行 `pnpm dev:services:vant`、`pnpm dev:services:nutui` 或 `pnpm dev:services:varlet`。代理規則與環境變數請參閱[請求架構](https://fonghehe.github.io/vue-h5-template/zh-TW/guide/v2/request)。
...
Production target 為 Chrome / Android WebView 111+ 與 Safari / iOS WebView 16.4+，支援相同核心的近期 WeChat 與 WeCom WebView；不支援 IE 與舊版 Android WebView。
