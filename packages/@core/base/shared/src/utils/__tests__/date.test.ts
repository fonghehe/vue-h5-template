...
describe("dateUtils", () => {
  const sampleISO = "2026-10-30T12:34:56Z";
  ...
  describe("formatDate", () => {
    it("should format a valid ISO date string", () => {
      ...
      expect(formatted).toMatch(/2026\/10\/30/);
    });
  ...
  describe("formatDateTime", () => {
    it("should format date into full datetime", () => {
      ...
      expect(result).toMatch(/2026-10-30 \d{2}:\d{2}:\d{2}/);
    });
  });
  ...
  describe("isDate", () => {
    ...
    it("should return false for non-Date values", () => {
      expect(isDate("2026-10-30")).toBe(false);
  ...
  describe("setCurrentTimezone & getCurrentTimezone", () => {
    ...
    it("should update dayjs default timezone", () => {
      ...
      const d = dayjs("2026-01-01T00:00:00Z");
      // 校验时区转换生效（小时变化）
